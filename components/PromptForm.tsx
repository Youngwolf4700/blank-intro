...
const PromptForm: React.FC<PromptFormProps> = ({
  ...
  const applyLogoRevealTemplate = () => {
    setGenerationMode(GenerationMode.FRAMES_TO_VIDEO);
    setPrompt("A cinematic high-end crypto intro for 'BLANK' token. The cyan and purple glowing rings explode and morph into the stylized 'BLANK' logo. High-velocity motion, volumetric lighting, digital particle streaks, and a premium professional finish. 4K quality look, black background.");
    setModel(VeoModel.VEO);
    setAspectRatio(AspectRatio.LANDSCAPE);
  ...
  const applyAtmosphericIntroTemplate = () => {
    setGenerationMode(GenerationMode.TEXT_TO_VIDEO);
    setPrompt("A sleek and futuristic crypto world background. Flowing streams of digital energy in cyan and purple navigate through a dark void. Ethereal data nodes and floating geometric crystals. High-end cinematic visual style.");
      ...
      return (
        <div className="mb-3 p-4 bg-[#2c2c2e] rounded-xl border border-gray-700 flex flex-col items-center justify-center gap-4">
          ...
          <p className="text-[10px] text-gray-400 italic font-medium">
            Intro Tip: Upload the ring image as Start and 'BLANK' text as End.
          </p>
          {startFrame && !endFrame && (
  ...
  return (
    <div className="relative w-full">
      ...
      {isTemplatesOpen && (
        <div className="absolute bottom-full left-0 mb-3 w-72 p-2 bg-[#2c2c2e] rounded-xl border border-gray-700 shadow-2xl z-40 overflow-hidden" ref={templatesRef}>
           <div className="p-3 text-[10px] text-gray-400 uppercase tracking-widest font-bold border-b border-gray-700 mb-2">Intro Presets</div>
           <button 
             onClick={applyLogoRevealTemplate}
             className="w-full flex items-center gap-3 p-3 hover:bg-indigo-600/50 rounded-lg text-left transition-all group">
             <div className="w-10 h-10 bg-indigo-500/20 rounded-md flex items-center justify-center group-hover:bg-indigo-500/40 transition-colors">
               <SparklesIcon className="w-5 h-5 text-indigo-400" />
             </div>
             <div>
               <div className="text-sm font-semibold text-white">Blank Logo Reveal</div>
               <div className="text-[10px] text-gray-400">Perfect for the 2 images</div>
             </div>
           </button>
           <button 
             onClick={applyAtmosphericIntroTemplate}
             className="w-full flex items-center gap-3 p-3 hover:bg-purple-600/50 rounded-lg text-left transition-all group mt-1">
             <div className="w-10 h-10 bg-purple-500/20 rounded-md flex items-center justify-center group-hover:bg-purple-500/40 transition-colors">
               <TvIcon className="w-5 h-5 text-purple-400" />
             </div>
             <div>
               <div className="text-sm font-semibold text-white">Future Atmosphere</div>
               <div className="text-[10px] text-gray-400">Sleek background visuals</div>
             </div>
           </button>
        </div>
      )}
      ...
      <form onSubmit={handleSubmit} className="w-full">
        {renderMediaUploads()}
        <div className="flex items-end gap-2 bg-[#1f1f1f] border border-gray-600 rounded-2xl p-2 shadow-lg focus-within:ring-2 focus-within:ring-indigo-500 transition-shadow">
          ...
          <button
            ...
            className={`p-2.5 rounded-full hover:bg-gray-700 transition-colors ${isTemplatesOpen ? 'bg-indigo-600/40 text-indigo-400 shadow-[0_0_15px_rgba(79,70,229,0.3)]' : 'text-gray-300'}`}
            title="Intro Presets">
          ...
          <div className="relative group">
            <button
              type="submit"
              className="p-2.5 bg-indigo-600 rounded-full hover:bg-indigo-500 disabled:bg-gray-600 disabled:cursor-not-allowed transition-all"
        ...
        <p className="text-[10px] text-gray-500 text-center mt-2 px-4 uppercase tracking-tighter">
          Click the Sparkles icon for the "Blank Logo Reveal" preset — perfect for the images you provided!
        </p>
      </form>
    </div>
  );
